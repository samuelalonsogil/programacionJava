...
public class QuickSort {
    private static int particion(int arra[], int izquierda, int derecha) {
		...
		while (true) {
			...
			/*
			 * Si la izquierda es mayor o igual que la derecha significa que no necesitamos
			 * hacer ningún intercambio de variables, pues los elementos ya están en orden
			...
			if (izquierda >= derecha) {
				...
				return derecha;
			} else {// Nota: yo sé que el else no hace falta por el return de arriba, pero así el
					// algoritmo es más claro
				/*
				 * Si las variables quedaron "lejos" (es decir, la izquierda no superó ni
				 * alcanzó a la derecha) significa que se detuvieron porque encontraron un valor
				 * que no estaba en orden, así que lo intercambiamos
				 */
	...
	private static void quicksort(int arra[], int izquierda, int derecha) {
		...
		if (izquierda < derecha) {
			...
			int indiceParticion = particion(arra, izquierda, derecha);

			quicksort(arra, izquierda, indiceParticion);
			quicksort(arra, indiceParticion + 1, derecha);
		}
	}
